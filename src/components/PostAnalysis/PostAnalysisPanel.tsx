...
function normalizeAnalysis(raw: Record<string, unknown>): PostAnalysis {
  ...
  if (raw.keystroke_breakdown && typeof raw.keystroke_breakdown === "object") {
    breakdown = raw.keystroke_breakdown as Record<string, number>;
  }
  if (Object.keys(breakdown).length === 0) {
    breakdown = { normal: 70, suspicious: 20, auto_complete: 10 };
  }
  return {
    id: String(raw.id ?? ""),
...
export const PostAnalysisPanel: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const load = async (attempt = 0) => {
      ...
      try {
        const res = await InterviewService.getPostAnalysis(sessionId);
        if (cancelled) return;
        ...
        setError(res.message || "Failed to fetch analysis");
        setLoading(false);
      } catch (e: unknown) {
        if (cancelled) return;
  ...
  if (loading) {
    return (
      <div className="post-analysis-loading">
        {pendingPoll ? "Analysis is running… this may take a few minutes." : "Loading analysis…"}
      </div>
    );
  }
  if (error) {
    return (
      <div className="post-analysis-error">
        <p>Error: {error}</p>
        {sessionId && (
          <Link to={`/interviews/${sessionId}`} style={{ color: "#065F46" }}>
            Back to interview
          </Link>
        )}
      </div>
  ...
  if (analysis?.status === "pending") {
    return (
      <div className="post-analysis-loading">
        Analysis is being processed. Please check back shortly…
      </div>
    );
  }
